...
const parentSignupController = async (req, res) => {
    ...
    if (checkEmail) {
        res
            .status(401)
            .json({ auth: false, token: null, message: "Email already exist." });
        return;
    }
    ...
    try {
        ...
        res.status(201).json({ id: userId, message: "Signup successful" });
    } catch (error) {
        console.log(error);
    }
};
